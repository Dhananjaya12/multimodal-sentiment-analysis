...
def extract_vision_features(image: Image.Image, crop_tightness: float = None):
    """Return a penultimate ResNet-style visual embedding for feature-level fusion."""
    if image is None:
        return None
    ...
    try:
        ...
        with torch.no_grad():
            if hasattr(model, "children"):
                ...
                if len(children) > 1:
                    feature_extractor = torch.nn.Sequential(*children[:-1]).to(device)
                    features = feature_extractor(image_tensor)
                    return features.reshape(features.shape[0], -1).squeeze(0).cpu()

            outputs = model(image_tensor)
            return outputs.reshape(outputs.shape[0], -1).squeeze(0).detach().cpu()
    except Exception as e:
        logger.error(f"Error extracting vision features: {str(e)}")
        return None
